...
def get_aws_regions():
    """Busca a lista de todas as regiões EC2 disponíveis."""
    try:
        return boto3.session.Session().get_available_regions('ec2')
    except Exception as e:
        st.warning(f"Não foi possível buscar a lista de regiões da AWS. Usando uma lista padrão. Erro: {e}")
...
def analyze_orphaned_volumes(regions_to_check):
    """
    ...
    for i, region in enumerate(regions_to_check):
        ...
        try:
            ec2_client = boto3.Session(profile_name=aws_profile_name).client('ec2', region_name=region)
...
run_analysis = st.button("🚀 Buscar Volumes Órfãos")
...
if run_analysis:
    if not selected_regions:
        ...
        df_orphaned = analyze_orphaned_volumes(selected_regions)
        
        st.header("📊 Resultados da Análise")
        
        if df_orphaned.empty:
            st.success("✅ Nenhuma volume EBS órfão encontrado nas regiões selecionadas. Bom trabalho!")
        else:
    ...
    st.info("Selecione as regiões na barra lateral e clique em 'Buscar Volumes Órfãos' para iniciar a análise.")